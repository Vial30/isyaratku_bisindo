# ==============================================================================
# LOCAL TRAINING SCRIPT - DEEP RESIDUAL BI-LSTM FUSION FOR WL-BISINDO
# (HOLISTIC BODY/FACE ANCHORS + HAND KEYPOINTS: 282-DIM INPUT)
# Directory: C:\TA\Mobile\model
# Dataset: C:\TA\Mobile\model\dataset_keypoints (282-dim .npy files)
# Hardware: NVIDIA GeForce GTX 1650 (CUDA)
# ==============================================================================
...
print(f"[INFO] Perangkat Komputasi Lokal: {device}")
if torch.cuda.is_available():
    print(f"[INFO] Nama GPU: {torch.cuda.get_device_name(0)}")

MODEL_DIR = r"C:\TA\Mobile\model"
DATASET_DIR = os.path.join(MODEL_DIR, "dataset")
KEYPOINTS_DIR = os.path.join(MODEL_DIR, "dataset_keypoints")

label_mapping_path = os.path.join(MODEL_DIR, "checkpoints", "labels_mapping.json")
if os.path.exists(label_mapping_path):
    with open(label_mapping_path, 'r', encoding='utf-8') as f:
        LABEL_MAPPING = json.load(f)
else:
    LABEL_MAPPING = {str(i): {"gloss": f"Class_{i}"} for i in range(32)}


# ==============================================================================
# 1. DATASET LOADER (HOLISTIC 282-DIM: BODY ANCHORS + HAND KEYPOINTS)
# ==============================================================================

class KeypointBISINDODataset(Dataset):
    """
    Dataset loader untuk Holistic 282-dim keypoint sequences.
    Memuat file .npy (shape: [16, 282]) per sample video.
    """
    def __init__(self, keypoints_dir, seq_len=16, feature_dim=282):
        ...
        print(f"\n[INFO] Dataset Holistic 282-Dim Berhasil Dimuat")
        ...
        print(f"       Feature Dim   : {self.feature_dim} (Body Anchors + Local/Global Hand + Velocity)\n")
    ...
    def __getitem__(self, idx):
        ...
        try:
            keypoints = np.load(npy_path).astype(np.float32)  # [16, 282]
        except Exception:
            keypoints = np.zeros((self.seq_len, self.feature_dim), dtype=np.float32)

        if keypoints.shape != (self.seq_len, self.feature_dim):
            keypoints = np.zeros((self.seq_len, self.feature_dim), dtype=np.float32)

        frames = torch.from_numpy(keypoints)  # [16, 282]
        return frames, label, signer


class AugmentedKeypointSubset(Dataset):
    """
    Augmentasi pada Holistic 282-dim keypoints:
    - Random rotation 2D (±15°)
    - Random scale (0.85 - 1.15)
    - Random translation (±0.05)
    - Gaussian noise (σ=0.01)
    - Rekomputasi vektor kecepatan
    """
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            # Ambil komponen posisi (141)
            pos = frames[:, :141].view(frames.shape[0], 47, 3).clone()  # 47 landmarks x 3

            # 1. Random 2D rotation (±15°)
            ...
            pos_flat = pos.view(frames.shape[0], 141)
            
            # Rekomputasi Kecepatan setelah augmentasi
            vel_flat = torch.zeros_like(pos_flat)
            vel_flat[1:] = pos_flat[1:] - pos_flat[:-1]

            frames = torch.cat([pos_flat, vel_flat], dim=-1)  # [16, 282]

        return frames, label, signer


# ==============================================================================
# 2. ARSITEKTUR MODEL (DEEP RESIDUAL BI-LSTM FUSION — 282-DIM VERSION)
# ==============================================================================

class KeypointProjection(nn.Module):
    """Memproyeksikan vektor keypoint 282-dim ke feature space 256-dim."""
    def __init__(self, input_dim=282, feature_dim=256):
...
class TemporalAttention(nn.Module):
    """Temporal Attention Mechanism — IDENTIK dengan Munsarif."""
...
class DeepResidualBiLSTMClassifier(nn.Module):
    """
    Arsitektur Deep Residual Bi-LSTM Fusion — IDENTIK dengan Munsarif.
    Hanya DeepSpatialNetwork (ResNet-18) diganti KeypointProjection 282-dim.
    """
    def __init__(self, num_classes=32, input_dim=282, feature_dim=256, hidden_dim=128, dropout_rate=0.5):
...
# ==============================================================================
# 3. LOSO CROSS-VALIDATION PELATIHAN LOKAL
# ==============================================================================

def train_loso_cv(dataset, num_classes=32, epochs=100, batch_size=16, lr=0.001, start_fold=0, patience=15):
    test_signers = []
    for s in sorted(np.unique(dataset.signers)):
        ...
        if len(s_labels) == num_classes:
            ...
            print(f"[INFO] Signer-{s} hanya memiliki {len(s_labels)}/{num_classes} kelas kata. Digunakan sebagai data latih tambahan.")
    ...
    progress_path = os.path.join(MODEL_DIR, "loso_progress.pth")
    ...
    print("======================================================================")
    print("MULAI PELATIHAN LOKAL: LEAVE-ONE-SUBJECT-OUT (LOSO) CROSS-VALIDATION")
    print("ARSITEKTUR: Holistic Body Anchors + Hand Keypoints (282-dim) -> Bi-LSTM Fusion")
    print(f"Total Signer Uji (32 Kelas): {total_folds} | Total Epochs per Fold: {epochs} | Patience: {patience}")
    print("======================================================================\n")
    ...
    for fold_idx in range(start_fold, total_folds):
        ...
        model = DeepResidualBiLSTMClassifier(num_classes=num_classes, input_dim=282).to(device)
        ...
        for epoch in range(0, epochs):
            ...
            pbar = tqdm(
                train_loader,
                desc=f"[Fold {fold_idx+1}/{total_folds}] Epoch {epoch+1:02d}/{epochs:02d}",
                unit="batch",
                ncols=100
            )
            ...
            print(f" -> [SELESAI Epoch {epoch+1:02d}/{epochs:02d}] Train Loss: {epoch_loss:.4f} | Train Acc: {epoch_acc*100:.2f}% | Test Acc (Signer-{test_signer}): {test_acc*100:.2f}% (Best: {max(best_fold_acc, test_acc)*100:.2f}%)", flush=True)
            ...
            if patience_counter >= patience:
                print(f"\n   [EARLY STOPPING] Tidak ada peningkatan akurasi selama {patience} epoch. Dihentikan di Epoch {epoch+1}.\n", flush=True)
                break
        ...
        loso_progress = {
            ...
            'best_overall_model_state': best_overall_model_state
        }
        torch.save(loso_progress, os.path.join(MODEL_DIR, "loso_progress.pth"))

    print(f"\n======================================================================")
    print(f"RATA-RATA AKURASI LOSO CROSS-VALIDATION: {np.mean(fold_results)*100:.2f}% (+/- {np.std(fold_results)*100:.2f}%)")
    print(f"======================================================================\n")

    save_path = os.path.join(MODEL_DIR, "best_bisindo_residual_bilstm.pth")
    if best_overall_model_state is not None:
        save_payload = {
            ...
            'input_type': 'holistic_body_hand_keypoint',
            'input_dim': 282,
            'seq_len': 16
        }
        torch.save(save_payload, save_path)
        print(f"[INFO] Model Terbaik Berhasil Disimpan ke: {save_path}\n")

    best_model_obj = DeepResidualBiLSTMClassifier(num_classes=num_classes, input_dim=282).to(device)
    if best_overall_model_state is not None:
...
# ==============================================================================
# 4. EVALUASI METRIK & SAVE HASIL GAMBAR BAB IV
# ==============================================================================
...
def evaluate_and_save(y_true, y_pred, y_prob, num_classes=32):
    ...
    print("======================================================================")
    print("METRIK EVALUASI HASIL LOKAL (UNTUK TABEL & PEMBAHASAN BAB IV)")
    print("======================================================================")
    ...
    json_path = os.path.join(MODEL_DIR, "evaluation_metrics_bab4.json")
    ...
    plt.title("Confusion Matrix - WL-BISINDO (Holistic Body/Face Anchors + Hand Keypoint)", fontsize=14)
    plt.xlabel("Predicted Label Class", fontsize=12)
    plt.ylabel("Actual Ground Truth Class", fontsize=12)
    plt.tight_layout()

    cm_path = os.path.join(MODEL_DIR, "confusion_matrix_bab4.png")
    plt.savefig(cm_path, dpi=300)
    plt.close()
    print(f"[INFO] Gambar Confusion Matrix PNG Disimpan ke: {cm_path}\n")


# ==============================================================================
# 5. PENGUJIAN LATENCY RESPONS WAKTU LOKAL (MOBILE BACKEND READINESS)
# ==============================================================================

def benchmark_latency(model, seq_len=16, input_dim=282, runs=50):
    if model is None:
        return
    model.eval()
    ...
    print("======================================================================")
    print("PENGUJIAN LATENCY RESPONS WAKTU LOKAL (PROTOTYPE MOBILE)")
    print("======================================================================")
...
if __name__ == "__main__":
    ...
    if len(y_true) > 0:
        ...
        benchmark_latency(trained_model, seq_len=16, input_dim=282)
